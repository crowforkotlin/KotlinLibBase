    ...
    fun <T> flowResult(intent: I, flow: Flow<T>, result: BaseMviFlowResult<I, T>) {
        viewModelScope.launch() {
            flow
                ...
                .catch { catch -> emitValueMoreoverDelayAfter(intent.also { it.mViewState = Error(if (catch is ViewStateException) Error.UNKNOW_HOST else Error.DEFAULT, msg = catch.message) }) }
                .collect {
                    runCatching { emitValueMoreoverDelayAfter(result.onResult(it).also { event -> event.mViewState = Result }) }
                        .onFailure { cause ->
    ...
    suspend fun <T> flowResult(flow: Flow<T>, intent: I? = null, context: CoroutineContext = Dispatchers.Main,  result: BaseMviFlowResult<I, T>) = suspendCancellableCoroutine { continuation ->
        viewModelScope.launch(context) {
            flow
                ...
                .catch { catch ->
                    trySendIntent(intent, Error (if (catch is ViewStateException) Error.UNKNOW_HOST else Error.DEFAULT, catch.message)) {
