...
import com.crow.base.android.app.app
    ...
    fun <T> flowResult(intent: I, flow: Flow<T>, result: BaseMviFlowResult<I, T>) {
        viewModelScope.launch {
            flow
                ...
                .catch { catch -> emitValueMoreoverDelayAfter(intent.also { it.mViewState =
                    Error(
                        if (catch is ViewStateException) Error.UNKNOW_HOST else Error.DEFAULT,
                        msg = catch.message ?: app.getString(R.string.BaseUnknowError)
                    )
                })}
